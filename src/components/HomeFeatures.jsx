  ...
  return (
    <div className="bg-[#f8f8f8] text-gray-900 font-poppins relative z-10">
            ...
            return (
              ...
              >
                <motion.img 
                  src={cat.img} 
                  alt={cat.title}
                  className="absolute inset-0 w-full h-full object-cover origin-center mix-blend-normal md:grayscale transition-all duration-700 group-hover:grayscale-0"
                  animate={{ 
                ...
                <div className="absolute inset-0 p-6 md:p-10 flex flex-col justify-end overflow-hidden">
                  <div className="flex flex-col h-full justify-end relative">
                      ...
                      {isHovered && (
                        ...
                        >
                          <div className="hidden md:block w-full">
                            ...
                            <p className="text-white/90 font-normal mb-8 max-w-md leading-relaxed text-lg">
                              {cat.desc}
                            </p>
                    ...
                    {/* Mobile always show title and subtitle */}
                    <div className="md:hidden block relative z-10 w-full flex-col justify-end">
                       <span className="text-[10px] font-bold text-white/90 uppercase tracking-[0.2em] block mb-1">{cat.subtitle}</span>
                       <h3 className="text-3xl font-black text-white">{cat.title}</h3>
                    </div>
      ...
      <section className="py-16 md:py-24 relative overflow-hidden">
        ...
        >
          {/* Tech Card 1 */}
          <motion.div variants={FadeUp} className="bg-white border border-gray-200 shadow-sm p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] hover:shadow-xl transition-all duration-300">
            <div className="w-12 h-12 bg-black rounded-full flex items-center justify-center text-white mb-6 md:mb-8">
              <Zap size={20} />
            </div>
            <h3 className="text-xl font-bold mb-2 md:mb-3 text-black">Zoom Air</h3>
          ...
          <motion.div variants={FadeUp} className="bg-white border border-gray-200 shadow-sm p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] hover:shadow-xl transition-all duration-300">
            ...
            <h3 className="text-xl font-bold mb-2 md:mb-3 text-black">Engineered Flyknit</h3>
            <p className="text-gray-600 text-sm leading-relaxed">
          ...
          {/* Tech Card 3 */}
          <motion.div variants={FadeUp} className="bg-white border border-gray-200 shadow-sm p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] hover:shadow-xl transition-all duration-300 sm:col-span-2 md:col-span-1">
            <div className="w-12 h-12 bg-black rounded-full flex items-center justify-center text-white mb-6 md:mb-8">
              <Target size={20} />
            </div>
            <h3 className="text-xl font-bold mb-2 md:mb-3 text-black">Waffle Grip Outsole</h3>
            <p className="text-gray-600 text-sm leading-relaxed">
      ...
      <section className="py-16 md:py-24 px-4 md:px-6 max-w-7xl mx-auto">
        <motion.div 
          ...
          viewport={{ once: true, margin: "-100px" }}
          variants={FadeUp}
          className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-10 md:mb-16 gap-4 md:gap-6"
        >
          <div>
            <div className='inline-flex items-center gap-2 bg-black/5 border border-black/10 rounded-full px-4 py-1.5 mb-6'>
        ...
        >
          {featuredProducts.map((product) => (
            <motion.div key={product.id} variants={FadeUp} className="group">
              <Link to={`/products/${product.id}`} className="block bg-[#f0f0f0] rounded-[1.5rem] md:rounded-[2rem] p-3 md:p-6 relative mb-3 md:mb-4 aspect-square flex items-center justify-center hover:bg-[#e8e8e8] transition-all">
                <img 
                  src={product.image} 
                  alt={product.name} 
              ...
              <div className="px-1 md:px-2">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-1">
                  <h3 className="font-bold text-sm md:text-lg tracking-tight text-black line-clamp-1">{product.name}</h3>
                  <span className="text-black font-black text-sm md:text-lg">${product.new_price}</span>
                </div>
                <span className="text-[10px] md:text-[12px] font-medium text-gray-500 capitalize block">{product.category}'s Shoes</span>
              </div>
            </motion.div>
          ))}
        </motion.div>
      ...
      <section className="py-16 md:py-24 px-4 md:px-6 max-w-7xl mx-auto mb-6 md:mb-10">
        <motion.div 
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
          variants={FadeUp}
          className="rounded-[2rem] md:rounded-[3rem] overflow-hidden bg-black text-white p-8 md:p-20 flex flex-col md:flex-row justify-between items-center gap-8 md:gap-12"
        >
          <div className="max-w-xl">
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tight leading-[1.05] mb-4 md:mb-6">Step Into The Future. Feel The Cushion.</h2>
            <p className="text-white/80 text-sm md:text-lg leading-relaxed mb-8 md:mb-10 font-normal">
            ...
            <div className="flex flex-wrap gap-3 md:gap-4">
              <Link to="/login" className="bg-white text-black font-bold px-6 py-3 md:px-8 md:py-4 rounded-full transition-all hover:scale-105 text-sm md:text-base">
                Join Now
              </Link>
              <Link to="/about" className="bg-white/10 border border-white/20 text-white font-bold px-6 py-3 md:px-8 md:py-4 rounded-full transition-all hover:bg-white/20 text-sm md:text-base">
                Our Story
              </Link>
            </div>
          </div>
          
          <div className="w-full max-w-[250px] md:max-w-[400px] flex justify-center items-center">
            <img 
